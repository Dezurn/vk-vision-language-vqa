...
Датасеты, исходные веса, LoRA-адаптеры, объединённая модель и временные чекпоинты намеренно исключены из Git из-за размера. Репозиторий содержит воспроизводимый код и итоговые табличные результаты.
...
## Что добавить перед публичной сдачей

В локальных файлах проекта нет следующих сведений; их нужно добавить после выбора способа публикации:

- ФИО автора, учебная группа и контакт — в начало README.
- Ссылка на архив с LoRA-весами и объединённой моделью либо публичные репозитории моделей на Hugging Face — в раздел «Артефакты решения».
- Лицензия авторского кода — отдельным файлом `LICENSE`.
- Ссылка на презентацию, если она будет приложена к работе.
- Финальная ссылка на GitHub или Облако Mail для формы обратной связи.
